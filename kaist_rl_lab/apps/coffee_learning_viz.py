...
LEARNING_VIZ_HTML = """
        <section id="ft-learning-viz" class="ft-learning-viz" aria-labelledby="ft-learning-title" hidden>
          ...
          <p class="hint ft-chart-explanation">Exploration includes random actions. Evaluation tests each updated policy
            without that noise. Points appear only when a rollout finishes; inspecting a point shows its results.</p>
        </section>
"""
...
.ft-chart-legend span {display:inline-flex;align-items:center;gap:5px;}
...
function createLearningVisualization(element) {
  ...
  const svg=$('#ft-learning-chart'),select=$('#ft-chart-trial'),zero=$('#ft-chart-zero');
  ...
  function phaseView(current,result,history) {
    ...
    const applied=history.filter(row=>finite(row.update?.actor_change)&&row.update.actor_change>0).length;
  ...
  function plot(result,history,width) {
    ...
    const observed=[...exploration,...evaluation,...best].map(point=>point.value);
    ...
    for(const [series,points] of [['exploration',exploration],['evaluation',evaluation],['best',best]]) {
      if(points.length)svgNode('path',{d:path(points,series==='best'),class:'ft-chart-series-'+series,'data-series':series});
    }
    const pointSets=[['exploration',exploration],['evaluation',evaluation]];
  ...
  function render(current) {
    ...
    const signature=JSON.stringify([result.baseline,history,result.episodes,zero.checked,selected,followLatest,width]);
    if(signature===lastSignature)return;
    plot(result,history,width);
    lastSignature=JSON.stringify([result.baseline,history,result.episodes,zero.checked,selected,followLatest,width]);
  }
  function reset() {
    state=null;selected=null;followLatest=true;lastSignature='';zero.checked=false;
    root.hidden=true;svg.replaceChildren();select.replaceChildren();select.disabled=true;
    const option=document.createElement('option');option.value='';option.textContent='No results yet';select.append(option);
    $('#ft-chart-inspector').replaceChildren();text($('#ft-learning-status'),'');text($('#ft-learning-updates'),'');
  }
