...
def test_finite_horizon_gae_uses_native_undiscounted_returns_and_zero_bootstrap():
    rewards = np.array([0.2, -0.1, 15.0])
    values = np.array([10.0, 12.0, 14.0])
    np.testing.assert_allclose(
        generalized_advantages(rewards, values, trace_decay=1),
        np.cumsum(rewards[::-1])[::-1] - values,
    )
    np.testing.assert_allclose(
        generalized_advantages(rewards, values, trace_decay=0),
        rewards + np.append(values[1:], 0) - values,
    )
